...
DataSource dataSource;
	...
	public int write(String userID, String boardTitle, String boardContent,String boardFile, String boardRealFile) {
		...
		String SQL = "INSERT INTO chatBoard SELECT ?, IFNULL((SELECT MAX(boardID) + 1 FROM chatBoard), 1), ?, ?, now(), 0, ?, ?, IFNULL((SELECT MAX(boardGroup) + 1 FROM chatBoard), 0), 0, 0, 1"; 
		try {
			...
			System.out.println("board write()메소드 오류"+e);
		} finally {
			try {
	...
	public BoardDTO getBoard(String boardID) {
		...
		PreparedStatement pstmt =  null;
		ResultSet rs = null;
		String SQL = "SELECT * from chatBoard WHERE boardID = ?"; 
		try {
			conn=dataSource.getConnection();
			pstmt = conn.prepareStatement(SQL);
			...
			System.out.println("getBoard()메소드 오류"+e);
		} finally {
			try {
				if(rs != null) rs.close();
				if(pstmt != null) pstmt.close();
	...
	public ArrayList<BoardDTO> getList(String pageNumber) {
		...
		PreparedStatement pstmt =  null;
		ResultSet rs = null;
			// boardGroup이 특정한 수치보다 큰 경우, 작은 경우에 한해서만 게시물 데이터를 가져오고 정렬을 진행함.
		String SQL = "SELECT * from chatBoard WHERE boardGroup > (SELECT MAX(boardGroup) FROM chatBoard) - ? AND boardGroup <= (SELECT MAX(boardGroup) FROM chatBoard) - ? ORDER BY boardGroup DESC, boardSequence ASC"; 
		try {
			conn=dataSource.getConnection();
			pstmt = conn.prepareStatement(SQL);
			...
			System.out.println("getList()메소드 오류"+e);
		} finally {
			try {
				if(rs != null) rs.close();
				if(pstmt != null) pstmt.close();
	...
	public int hit(String boardID) {
		...
		String SQL = "UPDATE chatBoard set boardHit = boardHit + 1 WHERE boardID = ?"; 
		try {
			conn=dataSource.getConnection();
			...
			System.out.println("board hit()메소드 오류"+e);
		} finally {
			try {
				if(pstmt != null) pstmt.close();
				if(conn !=null) conn.close();
	...
	public String getFile(String boardID) {
		...
		PreparedStatement pstmt =  null;
		ResultSet rs = null;
		String SQL = "SELECT boardFile from chatBoard WHERE boardID = ?"; 
		try {
			conn=dataSource.getConnection();
			pstmt = conn.prepareStatement(SQL);
			pstmt.setString(1, boardID);
			...
			System.out.println("getFile()메소드 오류"+e);
		} finally {
			try {
				if(rs != null) rs.close();
				if(pstmt != null) pstmt.close();
	...
	public String getRealFile(String boardID) {
		...
		PreparedStatement pstmt =  null;
		ResultSet rs = null;
		String SQL = "SELECT boardRealFile from chatBoard WHERE boardID = ?"; 
		try {
			conn=dataSource.getConnection();
			pstmt = conn.prepareStatement(SQL);
			pstmt.setString(1, boardID);
			...
			System.out.println("getRealFile()메소드 오류"+e);
		} finally {
			try {
				if(rs != null) rs.close();
				if(pstmt != null) pstmt.close();
	...
	public int update(String boardID, String boardTitle, String boardContent,String boardFile, String boardRealFile) {
		...
		String SQL = "UPDATE chatBoard set boardTitle = ?, boardContent = ?, boardFile = ?, boardRealFile = ? WHERE boardID = ?";
		try {
			conn=dataSource.getConnection();
			...
			System.out.println("board update()메소드 오류"+e);
		} finally {
			try {
				if(pstmt != null) pstmt.close();
				if(conn !=null) conn.close();
	...
	public int delete(String boardID ) {
		...
		String SQL = "UPDATE chatBoard SET boardAvailable = 0 WHERE boardID = ?";
		try {
			conn=dataSource.getConnection();
			...
			System.out.println("board delete()메소드 오류"+e);
		} finally {
			try {
				if(pstmt != null) pstmt.close();
				if(conn !=null) conn.close();
...
public int reply(String userID, String boardTitle, String boardContent,String boardFile, String boardRealFile, BoardDTO parent) {
	...
	String SQL = "INSERT INTO chatBoard SELECT ?, IFNULL((SELECT MAX(boardID) + 1 FROM chatBoard), 1), ?, ?, now(), 0, ?, ?, ?, ?, ?, 1"; 
	try {
		conn=dataSource.getConnection();
		...
		System.out.println("board reply()메소드 오류"+e);
	} finally {
		try {
			if(pstmt != null) pstmt.close();
			if(conn !=null) conn.close();
...
// 특정한 부모글, 어떠한 글에 댓글이 달리게 되면 시퀀스 값에 +1 해줌
public int replyUpdate(BoardDTO parent) {
	
	Connection conn = null;
	PreparedStatement pstmt =  null;
	
	String SQL = "UPDATE chatBoard SET boardSequence = boardSequence + 1 WHERE boardGroup = ? AND boardSequence > ?";
	try {
		conn=dataSource.getConnection();
		...
		System.out.println("board replyUpdate()메소드 오류"+e);
	} finally {
		try {
			if(pstmt != null) pstmt.close();
			if(conn !=null) conn.close();
...
// 숫자 없이 이전, 다음으로 페이징 처리
public boolean nextPage(String pageNumber) {
	Connection conn = null;
	PreparedStatement pstmt =  null;
	ResultSet rs = null;
	String SQL = "SELECT * from chatBoard WHERE boardGroup >= ?"; 
	try {
		conn=dataSource.getConnection();
		pstmt = conn.prepareStatement(SQL);
		pstmt.setInt(1, Integer.parseInt(pageNumber)*10);
...
public int targetPage(String pageNumber) {
	...
	PreparedStatement pstmt =  null;
	ResultSet rs = null;
	String SQL = "SELECT COUNT(boardGroup) from chatBoard WHERE boardGroup > ?"; 
	try {
		conn=dataSource.getConnection();
		pstmt = conn.prepareStatement(SQL);
